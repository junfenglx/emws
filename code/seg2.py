...
from numpy import exp, dot, zeros, outer, random, dtype, float32 as REAL, \
    ...
    ndarray, empty, sum as np_sum, prod, append as np_append, asanyarray, arange, multiply, copy as np_copy, \
    isnan as np_isnan
...
def full2halfwidth(uchar):
    """
    convert Chinese non han char to ASCII char
    ！ -> !
    :arg uchar
    """
    if uchar == u'':
...
class Seger(Word2Vec):
    ...
    def predict_sigle_position(self, sent, pos, prev2_label, prev_label):
        ...
        u = sent[pos] if pos < len(sent) else self.END

        pred_tuple = tuple([self.su_prefix + varient + u for varient in self.state_varient])
    ...
    def gen_unigram_bigram(self, sent, pos):
        """
        generate unigram and bigram
        :param sent: the sentence
        :param pos: current character position
        :return: _ represents - offset
        """
        u = sent[pos] if pos < len(sent) else self.END
        u_1 = sent[pos - 1] if pos > 0 else self.START
        u_2 = sent[pos - 2] if pos > 1 else self.START
        u1 = sent[pos + 1] if pos < len(sent) - 1 else self.END
        u2 = sent[pos + 2] if pos < len(sent) - 2 else self.END

        b_1, b_2, b1, b2 = (u_1, u), (u_2, u_1), (u, u1), (u1, u2)

        return u, u_1, u_2, u1, u2, b_1, b_2, b1, b2
    ...
    def _vocab_from_new(self, sentences):
        ...
        for sentence_no, sentence in enumerate(sentences):
            ...
            if sentence:
                ...
                subgrams = [char for char in char_seq] + [self.su_prefix + varient + char for char in char_seq for
                                                          varient in self.state_varient]
                bigrams = [char_seq[index] + char_seq[index + 1] for index in range(len(char_seq) - 1)]
                subgrams.extend(
                        [self.sb_prefix + varient + bigram for bigram in bigrams for varient in self.state_varient])
    ...
    def reset_weights(self):
        ...
        # default layer1_size = 100 in gensim, Seger changed to 50
        # REAL is float32
